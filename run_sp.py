...
def train(args):
    ...
    for i in range(1, env.num_agents):
        agents[env.agents[i]] = NFSPAgentPettingZoo(num_actions=env.action_space(env.agents[i]).n,
            state_shape=env.observation_space(learning_agent_name)["observation"].shape,
            hidden_layers_sizes=[64,64],
            q_mlp_layers=[64,64],
            device=device
            )
    eval_agents = {learning_agent_name: agent}
    for i in range(1, env.num_agents):
        print("ENV.AGENTS[i] IS ", env.agents[i])
        eval_agents[env.agents[i]] = RandomAgentPettingZoo(num_actions=env.action_space(env.agents[i]).n)
    ...
    with Logger(args.log_dir) as logger:
        for episode in range(args.num_episodes):
            ...
            if episode % args.evaluate_every == 0:
                average_rewards = tournament_pettingzoo(env, eval_agents, args.num_eval_games)
...
if __name__ == '__main__':
    ...
    args.log_dir = args.log_dir.replace("dqn", args.algorithm)
    print("LOGDIR IS ", args.log_dir)
